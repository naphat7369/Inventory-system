    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">ID: {asset.assetId} &bull; Category: {asset.category.name}</p>
        </div>
        {session?.role === 'ADMIN' && (
                  ...
                  <tbody>
                    {asset.repairLogs.map(log => (
                      <tr key={log.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 dark:bg-slate-800/50">
                        ...
                        <td className="p-4 text-sm text-gray-500 dark:text-gray-400">{log.resolution || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="flex items-center justify-between border border-[#D4D6CF] p-4 rounded-sm">
                  ...
                  {session?.role === 'ADMIN' && (
                    <form action={unlinkAsset.bind(null, asset.id)}>
                      <button type="submit" className="text-sm text-[#E24A22] font-medium hover:underline px-3 py-1 bg-white dark:bg-slate-900 rounded-sm transition-colors border border-[#E24A22]">
                        Unlink
                      </button>
                    </form>
                  )}
                        ...
                        <td className="p-4">
                          {session?.role === 'ADMIN' && (
                            <form action={unlinkAsset.bind(null, child.id)}>
                              <button type="submit" className="flex items-center gap-1 text-sm text-[#E24A22] bg-white dark:bg-slate-900 px-3 py-1 rounded-sm transition-colors border border-[#E24A22]">
